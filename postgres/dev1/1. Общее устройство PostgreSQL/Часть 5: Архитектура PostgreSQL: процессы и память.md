...
1. Локальная память процесса – хранит временные данные конкретного запроса.


2. Общая память сервера – используется всеми процессами для кеширования данных.


3. Буферный кэш (Shared Buffers) – основной кэш данных.


4. Операционная система (Page Cache) – ОС также кэширует дисковые данные.
...
1. Проверяет, есть ли данные в буфере (shared_buffers).


2. Если данных нет, загружает их с диска.


3. После изменения данные сначала пишутся в буфер, затем сбрасываются в файлы.
...
1. Клиент запрашивает данные (SELECT * FROM orders).


2. PostgreSQL сначала ищет их в буфере (shared_buffers).


3. Если данных нет, загружает с диска и кеширует в буфере.
...
1. PostgreSQL сначала записывает изменения в WAL (pg_wal).


2. Затем измененные данные обновляются в буфере.


3. Только после этого сбрасываются на диск.
...
1. Большое количество подключений → высокая нагрузка.
...
3. Операционная система кэширует файлы.
